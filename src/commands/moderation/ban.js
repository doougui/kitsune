...
module.exports = {
  ...
  async execute (client, message, args) {
    ...
    try {
      await guildMember.send(`\`\`🚔\`\` Você foi banido do servidor **${message.guild.name}**, mais informações abaixo.`, embedPunish);
      client.logger.log(`Successfully sent a message to ${guildMember.displayName} with ban details.`);
    } catch (error) {
      client.logger.warn(`Failed to send direct message to ${guildMember.displayName} with ban details. ${error}`);
    }

    try {
      await guildMember.ban(`Motivo: ${reason} | Punido por: ${message.author.tag}`);

      client.logger.log(`${message.author.username} successfully banned ${guildMember.displayName} from the server ${message.guild.name}`);
      ...
      client.logger.warn(`${message.author.username} failed to ban ${guildMember.displayName} from the server ${message.guild.name}. ${error}`);
    }
  },
